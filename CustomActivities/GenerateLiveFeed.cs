...
{
    public sealed class GenerateLiveFeed : CodeActivity<Model.Submission>
    {
        ...
        protected override Model.Submission Execute(CodeActivityContext context)
        {
            ...
            settings.FeedGenerationType = FeedType.Live;
            ...
            {
                Status = "Generating Live Feed",
                Date = DateTime.Now,
                Pass = true
            };
            submission.Statuses.Add(status);
            LogMessage.AddStatus(submission.TransactionId, stepId, status);
            submission.LiveInstallerUrls = FeedInterface.GetInstallers(xmlDoc, submission.AppId);
            return submission;
        }
    }
}
